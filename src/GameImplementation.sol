    ...
    PausableUpgradeable
{
    ...
    modifier checkTimeout() {
        if (
            ...
            block.timestamp > lastActionTime + moveTimeout
        ) {
     ...
     */
    function submitBoard(bytes32 boardCommitment, bytes calldata zkProof) 
        external 
        onlyPlayer 
        checkTimeout
    {
     ...
     */
    function makeShot(uint8 x, uint8 y) 
        external 
        onlyPlayer 
     ...
     */
    function submitShotResult(uint8 x, uint8 y, bool isHit, bytes calldata zkProof) 
        external 
        onlyPlayer 
        inState(GameState.Active)
        checkTimeout
    {
        ...
        bytes32 boardCommitment = target == player1 
            ? player1BoardCommitment 
            : player2BoardCommitment;
     ...
     */
    function verifyGameEnd(bytes32 boardCommitment, bytes calldata zkProof) 
        external 
        onlyPlayer 
        inState(GameState.Active)
        checkTimeout
    {
        ...
        bytes32 expectedCommitment = opponent == player1 
            ? player1BoardCommitment 
            : player2BoardCommitment;
        ...
        bool isValid = ZKVerifier(zkVerifier).verifyGameEnd(
            boardCommitment,
            shotHistoryHash,
            zkProof
        );
     ...
     */
    function setMoveTimeout(uint256 newTimeout) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
    ...
    function _updateGameState() internal {
        ...
        if (
            player1BoardCommitment != bytes32(0) && 
            player2BoardCommitment != bytes32(0) &&
            state != GameState.Active
        ) {
            state = GameState.Active;
        ...
        // If one player submitted but not both, move to Setup state
            ...
            (player1BoardCommitment != bytes32(0) || player2BoardCommitment != bytes32(0)) &&
            state == GameState.Created
        ) {
     ...
     */
    function _verifyBoardPlacement(bytes32 boardCommitment, bytes calldata proof) 
        internal 
        view 
        returns (bool) 
    {
    ...
    {
        // Call to the ZKVerifier contract
        return ZKVerifier(zkVerifier).verifyShotResult(
            boardCommitment,
            x,
            y,
            claimedHit,
